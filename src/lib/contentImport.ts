 ...
 * from a pasted list of names instead (e.g. copied from the "My Content" page).
 *
 * Pasted text doesn't use our display names exactly (lines carry extra words),
 * so everything goes through a fuzzy matcher and the user reviews the result
 * before it is applied.
 */

export interface ContentMatch {
  /** The pasted line the match came from. */
  source: string;
...
// Filler words that would otherwise match inside unrelated names.
...
// source like "charlotte" can't match a name that merely shares a short word.
...
/**
 * Match free-form sources (pasted lines) to candidate names.
 *
...
export function matchNames(sources: string[], candidates: string[]): MatchResult {
  ...
  for (const source of sources) {
    ...
    if (matches.length === 0) {
      // Keep every name tied for best coverage rather than guessing: a source

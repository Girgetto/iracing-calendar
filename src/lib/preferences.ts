...
/**
 * Get track frequency data for unowned tracks.
 * Returns a sorted list of tracks with how many times they appear and in which series.
 */
export function getUnownedTrackFrequency(
  allSeries: Series[],
  ownedTracks: string[]
): Array<{
  track: string;
  ...
  for (const s of allSeries) {
    for (const week of s.schedule) {
      if (!week.track || ownedTracks.includes(week.track)) continue;
